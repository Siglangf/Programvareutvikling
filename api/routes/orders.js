...
router.post('/insertOrder', async (req, res) => {
  const buyerID = req.body.state.highestBidderID;
  const sellerID = req.body.state.sellerID;
  const productID = req.body.state.productID;
  const ratedByBuyer = 0;
  const ratedBySeller = 0;

  const userValueArray = [buyerID, sellerID, productID, ratedByBuyer, ratedBySeller];

  const sqlquery = ("INSERT INTO orders (buyerID, sellerID, productID, ratedByBuyer, ratedBySeller) VALUES ?");
...
router.post('/updateRating', async (req, res) => {
  ...
  const sqlquery = "UPDATE orders SET ratedBy" + ratingUserType + " = 1 WHERE orderID = " + orderID + "; UPDATE user SET rating = rating + " + ratingValue + "WHERE userID = " + ratedUserID + ";" 
...
router.get('/orders', async (req, res) =>{
  ...
  const sqlquery = ("SELECT * FROM orders WHERE buyerID = " + userID + " OR sellerID = " + userID);
